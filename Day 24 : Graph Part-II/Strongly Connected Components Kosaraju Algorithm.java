...
public class Solution {
    ...
    public static void revDfs(int node ,  List<Integer> list , ArrayList<ArrayList<Integer>> transpose , boolean[] vis){
        vis[node] = true;
        list.add(node);
        for(int it : transpose.get(node)){
            if(!vis[it])    revDfs(it ,list , transpose ,  vis);
        }
    }
    ...
    public static void dfsTopo(int node , ArrayList<ArrayList<Integer>> adj , boolean[] vis , Stack<Integer> st){
        ...
        for(int it : adj.get(node)){
            if(!vis[it])    dfsTopo(it,adj,vis,st);
        }
        st.push(node);
    }
    ...
    public static List<List<Integer>> stronglyConnectedComponents(int n, int[][] edges) {
        ...
        while(!st.isEmpty()){
            ...
            if(!vis[node]){
                ...
                revDfs(node , list , transpose , vis);
                ans.add(list);
            }
        }
        return ans;
