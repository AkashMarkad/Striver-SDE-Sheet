...
public class Solution {
    public static ArrayList<ArrayList<Integer>> solveNQueens(int n) {
        ...
        solve(0,board,res,rowL,lowerD,upperD);
        return res;
        
    }
    public static void solve(int col,int board[][],ArrayList<ArrayList<Integer>> res, int rowL[],int lowerD[] , int upperD[]){
        if(col == board.length){
            res.add(construct(board));
            return;
        }
        for(int row=0;row<board.length;row++){
            if(rowL[row]==0 && lowerD[row+col]==0 && upperD[board.length-1+col-row]==0){
                ...
                lowerD[row+col]=1;
                board[row][col]= 1;
                solve(col+1,board,res,rowL,lowerD,upperD);
                board[row][col]=0;
                lowerD[row+col]=0;
                upperD[board.length-1+col-row]=0;
                rowL[row]=0;
